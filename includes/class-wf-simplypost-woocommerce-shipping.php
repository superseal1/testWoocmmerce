...
class wf_simplypost_woocommerce_shipping_method extends WC_Shipping_Method {
  ...
  private function get_result( $weight,  $price, $countryCode ) {
    $path = rtrim($this->shipping_site, '/') . '/';
    $url = $path.'api/gateway/v1/services?weight='.strval($weight).'&price='.strval($price).'&countryCode='.$countryCode.'&platform=woocommerce';
  ...
  private function process_result( $result = '' ) {
    if ( $result ) {
      ...
      foreach ( $rate_reply_details as $shippingMethod ) {
        ...
        $rate_id   =  strval( $shippingMethod->code );
        $rate_code = strval( $shippingMethod->code );
        $rate_cost = floatval( $shippingMethod->price );
        $this->prepare_rate( $rate_code, $rate_name, $rate_cost, $rate_id );
      }
    } else {
      return false;
    }
  ...
  private function prepare_rate( $rate_code, $rate_name, $rate_cost, $rate_id ) {

    $this->found_rates[ $rate_id ] = array(
      'id'       => $rate_code,
      'label'    => $rate_name,
      'cost'     => $rate_cost,
      'meta_data' => array('ServiceCodeSimplypost' => $rate_code)
    );
  }
